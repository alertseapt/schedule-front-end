  ...
  <div v-if="showModal" class="modal-overlay" @click="handleModalClick">
    <div class="modal-content nfe-info-modal large" ref="modal" tabindex="-1">
      ...
      <div class="modal-header">
        <h3>
          <i :class="isBookingSchedule ? 'fas fa-bookmark' : 'fas fa-file-invoice'"></i>
          {{ isBookingSchedule ? 'Informações da Marcação' : 'Informações da NF-e' }}
        </h3>
      ...
      <div class="modal-body">
        ...
        <div v-if="activeTab === 'general'" class="tab-content">
          ...
          <!-- Observations Card -->
          <div :class="['info-item', 'observations-item', { 'booking-info-highlight': isBookingSchedule }]">
            <label>
              <i :class="isBookingSchedule ? 'fas fa-bookmark' : 'fas fa-sticky-note'"></i>
              {{ isBookingSchedule ? 'Informações da Marcação:' : 'Observações do Agendamento:' }}
            </label>
            <span v-if="getObservations() && getObservations().trim()" class="observations-text">
              {{ getObservations() }}
            </span>
            <span v-else class="no-observations">
              <i :class="isBookingSchedule ? 'fas fa-bookmark' : 'fas fa-info-circle'"></i>
              <em>{{ isBookingSchedule ? 'Agendamento de marcação criado sem observações' : 'Não há observações para este agendamento' }}</em>
            </span>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'products'" class="tab-content">
          ...
          <div v-else class="empty-state">
            <i :class="isBookingSchedule ? 'fas fa-bookmark' : 'fas fa-box-open'"></i>
            <h3 v-if="isBookingSchedule">Agendamento de Marcação</h3>
            <h3 v-else>Nenhum produto encontrado</h3>
            <p v-if="isBookingSchedule">
              Este é um agendamento de marcação. Os produtos serão adicionados quando a NF-e for processada.
            </p>
...
export default {
  ...
  computed: {
    // Verificar se é agendamento de marcação (definir primeiro)
    isBookingSchedule() {
      if (!this.nfeData) return false
      
      // É uma marcação (não efetivada) se:
      // 1. Status é 'Marcação' (marcações ainda não efetivadas)
      // 2. OU supplier é 'Agendamento de Marcação' (identificador específico de marcações)
      // 3. OU info.type é 'booking' (estrutura de dados de marcação)
      // 4. E não tem nfe_key real (nfe_key null, vazia, ou temporária)
      // 5. E não é tipo 'nfe_schedule' (já foi efetivada)
      
      const hasRealNfeKey = this.nfeData.nfe_key && 
      ...
      const isBooking = (this.nfeData.is_booking === true || 
                        this.nfeData.supplier === 'Agendamento de Marcação' ||
                        this.nfeData.info?.type === 'booking') && 
                       !hasRealNfeKey && 
                       !isEffectivated
  ...
  methods: {
    ...
    getSupplierInfo() {
      // Para marcações, não há fornecedor
      if (this.isBookingSchedule) {
    ...
    getObservations() {
      // Para marcações, verificar se há observações especiais
      if (this.isBookingSchedule) {
        const observations = []
        
        if (this.nfeData.observations) {
          observations.push(this.nfeData.observations)
        }
        
        // Adicionar informações da marcação
        observations.push('Este é um agendamento de marcação criado sem NF-e. Os detalhes da nota fiscal e produtos serão adicionados posteriormente.')
        
        const createdBy = this.getBookingCreatedBy()
        if (createdBy && createdBy !== 'Sistema') {
          observations.push(`Criado por: ${createdBy}`)
        }
        
        return observations.join('\n\n')
      }
      
      // Retornar as observações normais do agendamento ou string vazia se não houver
    ...
    getHistoryIcon(action) {
      ...
      if (actionLower.includes('efetivada') || actionLower.includes('effectivated')) {
        return 'fas fa-arrow-up text-success'
      } else if (actionLower.includes('criado') || actionLower.includes('created')) {
