  ...
  methods: {
    ...
    async loadAvailableClients() {
      try {
        ...
        if (window.globalClientsCache && window.globalClientsCache.clients) {
          console.log('🚀 ScheduleBookingModal: Usando cache global de clientes:', window.globalClientsCache.clients.length)
        ...
        console.log('⚠️ ScheduleBookingModal: Cache global não disponível, carregando via API...')
    ...
    tryAutoSelectClient() {
      ...
      if (this.availableClients.length === 1) {
        ...
        console.log('Estoque selecionado automaticamente (usuário tem acesso a apenas um):', this.availableClients[0].name)
        return
      }
    ...
    async createBookingSchedule() {
      ...
      try {
        ...
        const scheduleData = {
          ...
          is_booking: true, // Nova propriedade para identificar agendamentos de marcação
